...
def main():
    ...
    surface.fill("chartreuse")
    big_surface = pygame.Surface((64, 64)).convert()
    big_surface.fill('purple')
    colors = itertools.cycle(["black", "blue", 'green', "red"])
    words = itertools.cycle(["Any ", "style ", "will ", "do. "])
    chunks = [TextChunk(next(words), fonttype=i, fontsize=i + 20, fontcolor=next(colors)) for i in range(16)]
    surface = pygame.image.load("cow.png").convert()
    chunks.extend([
            TextChunk("This image is inline."), Chunk(surface, FLOAT_INLINE), TextChunk("See?"), LineBreak()
        ])
    chunks.extend([
        TextChunk("This image is left aligned."), Chunk(surface, FLOAT_BREAK), TextChunk("See?"), LineBreak()
    ])
    chunks.extend([
        TextChunk("This image is center aligned.", fontsize=36, fonttype=FONTTYPE_BOLD), Chunk(surface, FLOAT_BREAK_CENTER), TextChunk("See?"), LineBreak()
    ])
    chunks.extend([
        TextChunk("This image is right aligned."), Chunk(surface, FLOAT_BREAK_RIGHT), TextChunk("See?"), LineBreak()
    ])
    group = pygame.sprite.GroupSingle()
    group.add(ChunkRenderer(chunks, 640))
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.VIDEORESIZE:
                ...
                group.sprite.max_width = event.w
                group.sprite.rebuild()
